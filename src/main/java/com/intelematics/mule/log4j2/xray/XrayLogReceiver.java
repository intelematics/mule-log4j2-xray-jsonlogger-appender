...
public class XrayLogReceiver implements Runnable {
	...
	private XrayLogReceiver(String awsRegion) {
		this(XrayAgent.getInstance(awsRegion));
	}

	private XrayLogReceiver(XrayAgent agent) {
		xrayAgent = agent;
	}

	/** For testing only */
	static XrayLogReceiver getInstance(XrayAgent agent) {
		return new XrayLogReceiver(agent);
	}
	...
	private JsonLoggerTransaction getTransaction(JsonLoggerEntry entry) {
		...
		if (transaction == null) {
			...
			transactionExpiry
					.add(new ExpiryTransaction(transaction, timeNow.plusSeconds(EXPIRE_AFTER_SECONDS), correlationId));
		}

		if (DEBUG_MODE)
			log.info("## Xray keys to expire: " + transactionExpiry.size() + ", open transactions: "
					+ transactions.size());
	...
	private void removeExpiredItems(Instant timeNow) {
		...
		if (expiredKeys != null) {
			...
			if (DEBUG_MODE)
				log.info("## Xray keys to new keys: " + transactionExpiry.size() + ", open transactions: "
						+ transactions.size());
		}
	}
}
